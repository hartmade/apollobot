...
FallbackHandler = Callable[[str, dict[str, Any], httpx.AsyncClient], Awaitable[dict[str, Any]]]
...
async def get_with_retry(
    ...
    *,
    ...
    for attempt in range(max_retries + 1):
        ...
        logger.info(
            "Rate limited (429), retrying in %.1fs (attempt %d/%d)", delay, attempt + 1, max_retries
        )
...
def require_api_key(env_var: str, server_name: str) -> str:
    ...
    if not key:
        raise ValueError(f"API key required for {server_name}: set {env_var} environment variable")
    return key


def apollobot_user_agent() -> str:
    """Return a truthful contact string for public scientific API requests."""
    contact = os.getenv("APOLLOBOT_CONTACT_EMAIL", "").strip()
    identity = contact or "https://github.com/frontier-science-ai/apollobot"
    return f"ApolloBot/0.2 ({identity})"
