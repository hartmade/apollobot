...
class ResearchImplementor:
    ...
    async def _scaffold(self, session: Session) -> tuple[str, list[dict[str, Any]]]:
    ...
    async def _build(self, session: Session) -> tuple[str, list[dict[str, Any]]]:
    ...
    async def _test(self, session: Session) -> tuple[str, list[dict[str, Any]]]:
        ...
        for f in impl_dir.rglob("*.py"):
            if f.name != "__pycache__" and "test" not in f.name:
                impl_files.append(
                    {"path": str(f.relative_to(impl_dir)), "content": f.read_text()[:2000]}
                )
    ...
    async def _document(self, session: Session) -> tuple[str, list[dict[str, Any]]]:
    ...
    async def _package(self, session: Session) -> tuple[str, list[dict[str, Any]]]:
    ...
    async def _validate(self, session: Session) -> tuple[str, list[dict[str, Any]]]:
